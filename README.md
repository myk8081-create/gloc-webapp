...
- Deploy: Vercel 정적 배포
- 주요 화면: 로그인, 최초 비밀번호 변경, 관리자 대시보드, 대리점 관리, 내/타대리점 재고조회, 재고수정, 제품등록/수정, 발주관리, 발주신청, QR/카카오톡 안내문 생성
...
- 관리자는 `재고 수정` 화면에서 대리점을 선택한 뒤 제품별 현재 재고, 안전재고, 보관 위치를 저장할 수 있습니다.
- 대리점 계정은 `재고 수정` 화면에서 본인 대리점 재고만 수정할 수 있습니다.
...
- 대리점 계정은 내 재고와 타대리점 재고를 분리 조회할 수 있고, 재고수정과 발주 등록은 자기 `dealer_code`로만 처리됩니다.
- 관리자는 웹에서 전체 대리점 재고와 제품등록 정보를 등록/수정/삭제할 수 있습니다.
- 비활성 계정은 로그인할 수 없습니다.
