...
| dealer_code | 대리점 코드 |
| dealer_name | 대리점명 |
| dealer_discount_rate | 대리점 공통 할인율(%). 같은 dealer_code 담당자는 같은 할인율 사용 |
...
- 관리자는 `대리점` 화면에서 대리점별 공통 할인율(%)을 입력하거나 수정할 수 있습니다. 같은 `dealer_code`로 생성된 추가 담당자는 별도 할인율을 갖지 않고 대리점 공통 할인율을 그대로 사용합니다.
